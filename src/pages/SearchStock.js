...
export default function SearchStock() {
  ...
  const addStock = async (stockSymbol, stockName) => {
    ...
    if (!isFound) {
      try {
        axios
          .post(`${baseURL}api/watchlist/add`, addStockData)
          .then(async (res) => {
            if (res.status === 200) {
              ...
              await getPriceData(stockSymbol).then((res) => {
                  ...
                  <Fade top>
                    <div className="searchresItem">
                      ...
                      {isStockFound ? (
                        <div
                          onClick={() =>
                            removeStock(searchResults[index]["1. symbol"])
                          }
                        >
